            ...
            for j in eachindex(meshk)
                for m1 in eachindex(meshz1)
                    for m2 = eachindex(meshz2)
                        ...
                        zq1 = meshz1[m1]
                        zq2 = meshz2[m2]
                        vecp3 = safe_sqrt(externalP2)
                        vecp4 = im * safe_sqrt(externalP2 - P2)
                        veck2 = sqrt(1 - zk2^2) * sqrt(1 - zk1^2) * sqrt(k2)
                        veck3 = zk2 * sqrt(1 - zk1^2) * sqrt(k2)
                        veck4 = zk1 * sqrt(k2)
                        vecq3 = zq2 * sqrt(1 - zq1^2) * sqrt(q2)
                        vecq4 = zq1 * sqrt(q2)
                        pk = vecp3*veck3 + vecp4*veck4
                        pq = vecp3*vecq3 + vecp4*vecq4
                        qplus2 = P2/4 + q2 + pq
                        qminus2 = P2/4 + q2 - pq
                        pqplus = pq + P2/2 
                        pqminus = pq - P2/2
                        ...
                        # allweight = - 2 * z2^2 * weightzq1 * weightzq2 * weightq2 * q2 / (32 * pi^4) / (qplus2*Aqplus^2 + Bqplus^2) / (qminus2*Aqminus^2 + Bqminus^2)
