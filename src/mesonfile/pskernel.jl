    ...
    for s = 1:length(meshz)
        for j = 1:length(meshk)
            for m = 1:length(meshz)
                ...
                zk = meshz[s]
                zq = meshz[m]
                pk = safe_sqrt(P2) * sqrt(k2) * zk
                pq = safe_sqrt(P2) * sqrt(q2) * zq
                qplus2 = P2/4 + q2 + pq
                qminus2 = P2/4 + q2 - pq
                pqplus = pq + P2/2 
                pqminus = pq - P2/2
                ...
                for numy in eachindex(inty)
                    kq = sqrt(k2) * sqrt(q2) * (zk * zq + inty[numy] * sqrt(1 - zk^2) * sqrt(1 - zq^2))
                    kqplus = kq + pk/2
                    kqminus = kq - pk/2
                    
                    kernel[1, 1][outIndex, interIndex] += weighty[numy] * D(k2 + q2 - 2 * kq) * ( -4*(Bqminus*Bqplus + Aqminus*Aqplus*(-0.25*P2 + q2)) )
                    
                    kernel[1, 2][outIndex, interIndex] += weighty[numy] * D(k2 + q2 - 2 * kq) * ( 4*Aqminus*Bqplus*pqminus - 4*Aqplus*Bqminus*pqplus )
                    
                    kernel[1, 3][outIndex, interIndex] += weighty[numy] * D(k2 + q2 - 2 * kq) * ( 4*pq*(Aqminus*Bqplus*qqminus - Aqplus*Bqminus*qqplus) )
                    
                    kernel[1, 4][outIndex, interIndex] += weighty[numy] * D(k2 + q2 - 2 * kq) * ( 4*Aqminus*Aqplus*(pqplus*qqminus - pqminus*qqplus) )
